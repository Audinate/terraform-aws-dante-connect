...
module "dvs" {
  ...
  vpc_id                = "vpc-01234567890abcdef"
  licenseServerConfig   = {
    licenseServerHostname = "http://exampleABCD.seting.com"
    licenseServerApiKey   = "someApiKeyProvided"
  }
}
```
...
| <a name="input_install_reaper"></a> [install\_reaper](#input\_install\_reaper) | (Optionally) True to install REAPER on the DVS instance | `bool` | `false` | no |
...
| <a name="input_latency"></a> [latency](#input\_latency) | (Optionally) The latency threshold in ms. Allowed values = [4, 6, 10, 20, 40] | `number` | `10` | no |
| <a name="input_licenseServerConfig"></a> [licenseServerConfig](#input\_licenseServerConfig) | n/a | <pre>object({<br>    licenseServerHostname = string<br>    licenseServerApiKey   = string<br>  })</pre> | `null` | no |
...
| <a name="input_vpc_security_group_ids"></a> [vpc\_security\_group\_ids](#input\_vpc\_security\_group\_ids) | (Optionally) List of security group IDs the DVS instance will be associated with | `list(string)` | `null` | no |
