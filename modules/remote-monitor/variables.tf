...
variable "instance_type" {
  description = "(Optional) Instance type to use for the Remote Monitor instance. Updates to this field will trigger a stop/start of the instance"
  type        = string
  default     = "m5.large"
  nullable    = false
}
...
variable "vpc_security_group_ids" {
  description = "(Optional) List of security group IDs the Remote Monitor instance will be associated with"
  type        = list(string)
  nullable    = true
  default     = null
}
...
variable "subnet_id" {
  description = "The VPC Subnet ID the Remote Monitor instance will be launched in"
  type        = string
}

variable "vpc_id" {
  description = "The VPC ID the Remote Monitor instance will be created in"
  type        = string
}
...
variable "create_alb" {
  description = "(Optional) True if an ALB in front of the Remote Monitor must be created (to perform HTTPS termination)"
  type        = bool
  default     = false
}
...
variable "license_key" {
  description = "The Remote Monitor license provided by Audinate"
  type        = string
  sensitive   = true
  nullable    = true
  default     = null
